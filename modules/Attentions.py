...
class BilinearAttention(nn.Module):
    ...
    def score(self, query, key, query_mask=None, key_mask=None, mask=None, sum_attention=None):
        ...
        if query_mask is not None:
            attn = attn.masked_fill(~query_mask.unsqueeze(2).expand(batch_size, query_len, key_len), 0)
    ...
    def unnormalized_score(self, query, key, key_mask=None, mask=None, sum_attention=None):
        ...
        if key_mask is not None:
            attn = attn.masked_fill(~key_mask.unsqueeze(1).expand(batch_size, query_len, key_len), -float('inf'))
